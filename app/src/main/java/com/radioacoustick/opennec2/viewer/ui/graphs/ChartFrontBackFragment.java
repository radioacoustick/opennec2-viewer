...
/**
 * A fragment containing a MPAndroidChart widget for displaying Front/Back graph
 */
public class ChartFrontBackFragment extends Fragment {

	public ChartFrontBackFragment() {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View root = inflater.inflate(R.layout.fragment_chart_front_back, container, false);
		LineChart chartGain = root.findViewById(R.id.fb_line_chart);
		...
		viewModel.getNecResult().observe(getViewLifecycleOwner(), result -> {
			if (result != null && result.frequencies != null) {
				GraphsHelper.displayGraph(chartRenderer, result.frequencies, result.frontToBackF, getString(R.string.front_back) + " (dB)", false);
			}
		});
